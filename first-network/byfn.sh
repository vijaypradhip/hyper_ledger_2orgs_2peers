...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.supply.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.supply.com/ca && ls *_sk)
  fi
  ...
  #ADDed 
  sed -i -e 's/\r$//' scripts/script.sh
  sed -i -e 's/\r$//' scripts/utils.sh
  cd ..
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.supply.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.supply.com
    docker cp -a orderer.supply.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.supply.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.supply.com

    for PEER in peer0.org1.supply.com   peer0.org2.supply.com ; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.supply.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
